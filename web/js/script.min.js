  ...
  app.ready(function () {
    if (window.Bloodhound) {
      var a = new Bloodhound({
        ...
        prefetch: {url: app.dir.assets + "data/json/files.json", cache: !1},
      });
    ...
    "file:" == location.protocol &&
      app.toast(
        'Please open the page using "http" protocol for full functionality.',
        {duration: 15e3, actionTitle: "Read more", actionUrl: ""}
      );
    var b = [
      "primary",
      "secondary",
    ...
    $('[data-provide~="demo-color-changer"]').each(function () {
      ...
      $(document).on("click", ".demo-icons-list li", function () {
        ...
        (a += '<small class="sidetitle">COPIED</small>'),
          $("#icon-selected")
            .removeClass("text-secondary text-info")
      ...
      $(document).on("mouseleave", ".demo-icons-list", function () {
        $("#icon-selected")
          ...
          .text("Click an icon to copy the class name");
      }),
      ($.expr.pseudos.iconsSearch = function (a, b, c) {
        return (
          $(a)
            .dataAttr("clipboard-text")
            .toUpperCase()
            .indexOf(c[3].toUpperCase()) >= 0
        );
      }),
      $("#icons-search-input").on("keyup", function (a) {
        ...
        d || (c = $(".demo-icons-list li")),
          "" === b
            ? (c.show(), $("#icon-tabs li:eq(1) a").tab("show"))
            : (c.not(":iconsSearch(" + b + ")").hide(),
              c.filter(":iconsSearch(" + b + ")").show(),
              d &&
                ($("#tab-search-result ul").html(
                  c.filter(":iconsSearch(" + b + ")").outerHTML()
                ),
      ...
      $(document).on(
        "change",
        'input[name="global-topbar-color"]',
        function () {
          var a = $('input[name="global-topbar-color"]:checked').val();
          "default" == a
            ? $("body > .topbar")
                .removeClass("topbar-inverse")
                .css("background-color", "#fff")
            : $("body > .topbar")
                .addClass("topbar-inverse")
                .css("background-color", "#" + a);
        }
      ),
      $(document).on(
        "change",
        'input[name="global-sidebar-color"]',
        function () {
          var a = $('input[name="global-sidebar-color"]:checked').val();
          $(".sidebar").removeClass(
            "sidebar-light sidebar-dark sidebar-default"
          ),
      ...
      $(document).on(
        "change",
        'input[name="global-sidebar-menu-color"]',
        function () {
      ...
      $(document).on(
        "change",
        'input[name="sidebar-header-bg-color"]',
        function () {
      ...
      $(document).on("click", "#timeline-size-selector .btn", function () {
        var a = $(this).children("input").val();
        $("#demo-timeline-size").attr(
          "class",
          "timeline timeline-content-right timeline-point-" + a
        );
      });
  });
...
!(function (t, d, e, i, n) {
  (n = d.body).insertAdjacentHTML(
    "beforeend",
    '<div data-formito-id="efk3xf6OHZAtUfyYtv6l" data-trigger="fab" data-button-icon="https://s.formito.com/img/icon-fab/1.png"></div>'
  ),
